#ifndef PIPE_OS_SERVICES_IPC_PIPE_HPP
#define PIPE_OS_SERVICES_IPC_PIPE_HPP
...
namespace Pipe::os_services::ipc
{
	...
	public:
		...
		[[nodiscard]] auto close_read_end_on_exec()
		{
			...
			{
				throw error_handling::system_error{
					"Failed to set FD_CLOEXEC on pipe read end",
					error_handling::get_error_code()
		...
		[[nodiscard]] auto close_write_end_on_exec()
		{
			...
			{
				throw error_handling::system_error{
					"Failed to set FD_CLOEXEC on pipe write end",
					error_handling::get_error_code()
				};
			}
