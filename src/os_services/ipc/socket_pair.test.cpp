...
TESTCASE(Pipe_ipc_socket_pair_create_and_do_stuff)
{
	Pipe::os_services::ipc::socket_pair<SOCK_STREAM> socket_pair;
	std::string_view msg{"Hello, World"};
	auto write_result = Pipe::os_services::io::write(socket_pair.socket_a(), std::as_bytes(std::span{msg}));
	EXPECT_EQ(write_result.bytes_transferred(), 12);

	std::array<char, 12> msg_read{};
	auto read_result = Pipe::os_services::io::read(socket_pair.socket_b(), std::as_writable_bytes(std::span{msg_read}));
